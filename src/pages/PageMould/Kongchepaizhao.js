...
export default class Kongchepaizhao extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            bgImg: '',
    ...
    getlist = ()=>{
        const { location: { query: { data } } } = this.props
        getPhotoList({ 'picType': '2', 'allnums': data.allnums, 'orderid': data.orderid }).then(res => {
            if (res.success) {
                this.setState({
    ...
    shuangchuan = ()=>{
        ...
        fetch('/api/other-weixin/weixin/uppic/picupload_kc', {
            method: 'POST',
            headers: {
