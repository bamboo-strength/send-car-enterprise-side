...
@Form.create()
class DispatchplanView extends PureComponent {
  ...
  componentWillMount() {
    ...
    dispatch(TABLEEXTEND_COLUMNLIST({tableName:'mer_dispatchplan','modulename':'dispatchplan',queryType:3})).then(() => {
  ...
  render() {
    ...
    // console.log(detail.sublist,columns)
    return (
      <div>
        <NavBar
          mode="light"
          icon={<Icon type="left" />}
          onLeftClick={() => router.push('/dispatch/dispatchplan')}
        >{`查看${tableTitle}`}
        </NavBar>
