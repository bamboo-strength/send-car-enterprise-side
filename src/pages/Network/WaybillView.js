...
import { Card, Col, Form, Icon, List, Row } from 'antd';
import router from 'umi/router';
import { NavBar } from 'antd-mobile';
import { detail } from '@/services/FreightServices';
...
export class WaybillView extends PureComponent {
  ...
  render() {
    ...
    const imageDatas = func.notEmpty(freightPictureList) ? (
      freightPictureList.map((item, index) => {
        return <Row style={{flexDirection:'column',padding:10}}>
          ...
          <Col span={24} style={{ textAlign: 'center' }}>{index == 0 ? '发货单' : '签收单'}</Col>
        </Row>;
      })
    ) : undefined;
    ...
    const evaluation = [<Row style={{display:'flex',flexDirection:'column'}}>
      <Col span={24}>
        <NetWorkRate label='装货及时率' id='loadingEfficiencyScore' initialValue={scoreTypeName!=undefined?scoreTypeName.装货及时率:''} disabled form={form} />
      </Col>
      <Col span={24}>
        <NetWorkRate label='配合度' id='matchDegreeScore' initialValue={scoreTypeName!=undefined?scoreTypeName.配合度:''} disabled form={form} />
      </Col>
    </Row>]
