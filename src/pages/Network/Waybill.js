...
import { ListView, PullToRefresh, NavBar, Tabs, Toast } from 'antd-mobile';
import { Form, Icon, message, Modal, Radio, Checkbox } from 'antd';
import router from 'umi/router';
import func from '@/utils/Func';
import { connect } from 'dva';
import Text from 'antd/es/typography/Text';
import { list, orderTake, cancel, evaluation, orderTakeVerify, getShippingNoteInfos } from '@/services/FreightServices';
...
import { center, contract, sign, addSigner, signedContract } from '@/services/NetContract';
...
class Waybill extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      tabs: [{ title: '待执行', key: '1' }, { title: '执行中', key: '2' }, { title: '已完结', key: '3' }, {
        title: '已取消',
        key: '4',
      }],
      checked: false,
  ...
  pause = (e, rowData) => {  // 暂停
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      async onOk() {
        getShippingNoteInfos({id:rowData.id}).then((res)=>{
          if(res.success){
            const remark = res.data.remark
            delete res.data.remark
            TrajectoryMonitoring.pauseMonitoring(JSON.stringify(res.data),remark)
          }
        })
      },
      onCancel() {
  ...
  confirmRate = () => {
    ...
    form.validateFields((error, values) => {
      const { loadingEfficiencyScore, matchDegreeScore } = values;
      const evaluationScore = loadingEfficiencyScore + matchDegreeScore;
      if (loadingEfficiencyScore === null || Number.isNaN(loadingEfficiencyScore)) {
        Toast.offline('请选择装货及时率分数！');
        return;
      }
      ...
      const scoreType = {
        5: loadingEfficiencyScore,
        6: matchDegreeScore,
      };
      evaluation({ evaluationType: 2, waybillid: rowData.id, scoreType, evaluationScore }).then(resp => {
        if (resp.success) {
  ...
  render() {
    const { rowDataOne, dataSource, isLoading, refreshing, waybillStatus, rateShow, visibleCancel, load, cancelValue, tabs, order, checked,confirmLoading } = this.state;
    ...
    const row = (rowData, sectionID, rowID) => {
      ...
      if (aa === 0) { /* 待执行 */
        rowData.sourceGoodsType === 1 ?
          actions = ([
            <div onClick={(e) => this.cancel(e, rowData)}>取消</div>,
            <div style={{ color: '#1890FF' }} onClick={(e) => this.orderTake(e, rowData)}>接单</div>,
          ])
          :
          actions = ([
            <div style={{ color: '#1890FF' }} onClick={(e) => this.orderTake(e, rowData)}>接单</div>,
        ...
        actions = ([
          <div style={{ color: '#1890FF' }} onClick={(e) => this.submitLoad(e, rowData)}>装货确认</div>,
        ]);
      } else if (aa === 2) { /* 已装运 */
        actions = ([
          <div onClick={(e) => this.submitAgainLoad(e, rowData)}>重新上传装货信息</div>,
          <div style={{ color: '#1890FF' }} onClick={(e) => this.submitSign(e, rowData)}>卸货确认</div>,
          // <div style={{ color: '#1890FF' }} onClick={(e) => this.pause(e, rowData)}>暂停</div>,
          // <div style={{ color: '#1890FF' }} onClick={(e) => this.restart(e, rowData)}>重新开始</div>,
        ]);
      } else if (aa === 3) { /* 已签收 */
        actions = ([
          <div style={{ color: '#1890FF' }} onClick={(e) => this.submitAgainSign(e, rowData)}>重新上传卸货信息</div>,
          <div style={{ color: '#1890FF' }} onClick={(e) => this.evaluate(e, rowData)}>评价</div>,
        ]);
      }else if (aa === 4) { /* 已审核待支付 */
        actions = ([
          <div style={{ color: '#1890FF' }} onClick={(e) => this.evaluate(e, rowData)}>评价</div>,
        ...
        if (rowData.evaluationDriverStatus === 0) {
          actions = ([
            <div style={{ color: '#1890FF' }} onClick={(e) => this.evaluate(e, rowData)}>评价</div>,
          ]);
        }
    ...
    return (
      <div id={NetWorkLess.netWork}>
        ...
        <Modal visible={order} closable={false} className={NetWorkLess.netWorkModal} onCancel={this.hideModal} onOk={this.signMethod} confirmLoading={confirmLoading}>
          <Icon type="question-circle" style={{ color: '#faad14', marginRight: 16, fontSize: 22 }} />
          <div>
            <Text strong style={{ fontSize: 18 }}>确认接单</Text>
            <div style={{ marginTop: 10 }}>
              确定承接此条运单?
              <Checkbox onChange={a => this.agreeItem(a)} checked={checked} style={{ marginTop: 10 }}>
                同意
                <a onClick={(b) => {
                  b.preventDefault();
                  ...
                  router.push(
                    {
                      pathname: `/network/waybill/waybillagreement`,
                      state: {
                        data: rowDataOne,
                      },
                    },
