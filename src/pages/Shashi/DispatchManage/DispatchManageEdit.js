...
class CommonPageEdit extends PureComponent {
  ...
  componentWillMount() {
    ...
    dispatch(COMMONBUSINESS_DETAIL({realId:id, tableName,modulename})).then(()=>{
        ...
        let newArr =[]
      let ifCertainMaterial = '0' // 不确定
        if(func.notEmpty(detail.sublist)){
          newArr = detail.sublist.map((v,index)=>{    // 添加行号
            return {...v,'key':index+1}
          })
        //  console.log(detail.sublist[0].materialno,'===')
          ifCertainMaterial = detail.sublist[0].materialno?'1':'0'
        }
        this.setState({
          dataSource: newArr,
          ifCertainMaterial
...
//  提交按钮事件
  handleSubmit = e => {
    ...
    const  params = addSaveCommon(detail.id,form,dataSource,ziSaveFlag,showColums,showSubColums,false)
    const realId = params.id
    params.source = '1' // 客商添加
    // console.log(detail,params,'=============')
    if(func.notEmpty(params)){
      params.realId = realId
      if(params.ifCertainMaterial === "0"){ // 选择主合同
        params.sublist = []
        const subData = {grosspreamount:'0',id:detail.sublist[0].id,mainid:detail.sublist[0].mainid,materialno:'',contractSubno:''}
        params.sublist.push(subData)
      }else { // 选择子合同
        params.contractno = params.contractnomain?params.contractnomain:detail.contractno
...
// 修改组织机构和客户重新获取单价
    ...
    if (loadtype === "1") {
      ...
      queryDiscountPrice({tableName,modulename,btnCode: 'bringPriceMain',queryParam:params}).then((resp) => {
        if (resp.success && resp.data && resp.data.length) {
          const resData = resp.data.map(item => {
            return { ...item, rowKey: Math.random() }
          })
          this.setState({ dataSource: resData });
        }
  ...
  // 是否确定物资 切换主合同/子合同带回
  ifCertainMaterials=(form1,e)=>{
    const {form} = this.props
    form.resetFields()
    if(e.target.value === '0'){ // 不确定物资
      this.setState({
        dataSource:[],
        ifCertainMaterial:'0'
      })
  ...
  render() {
    ...
    const {dataSource,ifCertainMaterial} = this.state;
    ...
    // console.log(ifCertainMaterial,typeof ifCertainMaterial,'===ifCertainMaterial')
    let showSubColums = []
    ...
    detail.ifCertainMaterial = ifCertainMaterial
    detail.contractSubno = Array.isArray(detail.sublist)?detail.sublist[0].contractSubno:''
