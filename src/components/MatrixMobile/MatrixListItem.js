...
class MatrixListItem extends PureComponent {
  ...
  render() {
    ...
    return (
      <List.Item style={style} thumb={<div>{label}：</div>} className={`matrix-list-item matrix-list-item-${className}`}>
        {
          type ? <img src={title} alt="" className={`list-item-img-${className}`} onClick={()=>this.onClick(title)} />:<Text>{title}</Text>
        }
